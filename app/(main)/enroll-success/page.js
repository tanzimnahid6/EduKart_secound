...
const Success = async ({ searchParams: { session_id, courseId } }) => {
  ...
  if (!session_id)
    throw new Error("Please provide session_id thats start with 'cs_'.");
  ...
  // console.log(customerEmail, customerName, productName);

  if (paymentStatus === "succeeded") {
    
    //update db with enrollment details.
    const enrolled = await enrollForCourse(courseId, loggedInUser.id, "stripe");
    console.log(enrolled);
    
    //send mail to instructor about successful enrollment,student and the person who paid.
    ...
    // console.log(emailSentResponse[0]);
    ...
    <div className="h-full w-full flex-1 flex flex-col items-center justify-center">
      <div className="flex flex-col items-center gap-6 max-w-[600px] text-center">
        {paymentStatus == "succeeded" && (
          <>
            {" "}
            <CircleCheck className="w-32 h-32 bg-success rounded-full p-0 text-green-400" />
